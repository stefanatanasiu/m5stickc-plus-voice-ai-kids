 ...
 * - Button C (click): Next page for long answers
 */
...
const char* AZURE_OPENAI_KEY = "XXX";
const char* AZURE_OPENAI_ENDPOINT = "https://XYZ.openai.azure.com"; // No trailing slash
...
// ============================================================================
// OPTIONAL: AZURE TABLE LOGGING (direct device -> Table Storage)
// If you want the device to insert logs directly into a Table, set these values.
// Example table service URL: https://<account>.table.core.windows.net
// Example SAS token (short-lived or table-level SAS): sv=...&ss=... (without leading '?')
...
String response = "Press A to CONTINUE\nPress B for NEW topic";

// Pagination for long responses
const int LINES_PER_PAGE = 7; // number of text lines that fit above the button area
const int MAX_RESPONSE_LINES = 64;
String responseLines[MAX_RESPONSE_LINES];
...
// Forward declaration for escape helper (defined later)
...
void addToHistory(const String &role, const String &content) {
  ...
  String esc = escapeJSON(content);

  if (historyCount < MAX_HISTORY) {
    conversationHistory[historyCount] = "\"role\":\"" + role + "\",\"content\":\"" + esc + "\"";
    historyCount++;
  } else {
    ...
    conversationHistory[MAX_HISTORY - 1] = "\"role\":\"" + role + "\",\"content\":\"" + esc + "\"";
  }
...
// Escape a string for safe inclusion in JSON values (no surrounding quotes)
String escapeJSON(const String &s) {
  String r;
...
// Draw a single page of the prepared response and show button hints
void drawAnswerPage() {
  M5.Display.fillScreen(TFT_BLACK);
  M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
  M5.Display.setTextDatum(TL_DATUM);
  ...
  if (endLine > responseLineCount) endLine = responseLineCount;

  for (int i = startLine; i < endLine; i++) {
    M5.Display.drawString(responseLines[i], 3, y);
    y += 15;
  }
  ...
  // Left button (A) - Next page when viewing answer, otherwise Continue
  M5.Display.setTextColor(TFT_GREEN, TFT_BLACK);
  if (totalPages > 1) {
    M5.Display.drawString("A=Next", 5, 118);
  } else {
  ...
  if (totalPages > 1) {
    M5.Display.setTextColor(TFT_WHITE, TFT_BLACK);
    M5.Display.drawString(String(currentPage + 1) + "/" + String(totalPages), 100, 118);
  }

  showingAnswer = true;
}

// Prepare pagination: split wrapped response into lines and compute pages
void prepareResponsePagination(const String &wrapped) {
  responseLineCount = 0;
  String line;
  for (unsigned int i = 0; i <= wrapped.length(); i++) {
    char c = (i < wrapped.length()) ? wrapped[i] : '\n';
    if (c == '\n') {
      if (responseLineCount < MAX_RESPONSE_LINES) {
        responseLines[responseLineCount++] = line;
      }
      line = "";
    } else {
      line += c;
  ...
  if (responseLineCount == 0) {
    responseLines[0] = wrapped;
    responseLineCount = 1;
  }

  totalPages = (responseLineCount + LINES_PER_PAGE - 1) / LINES_PER_PAGE;
...
String transcribeAudio() {
  ...
  // Send HTTP request
  // Compute path portion reliably from the full URL and host
  String path = "/";
  int idx = url.indexOf(host);
  if (idx >= 0) {
    path = url.substring(idx + host.length());
  ...
  if (textIdx < 0) {
    // Not JSON with a "text" field - maybe the API returned plain text
    String trimmed = response;
    trimmed.trim();
    if (trimmed.length() > 0) {
      Serial.println("No JSON 'text' field; using raw response body as transcription.");
      Serial.println("Transcription: " + trimmed);
      Serial.println("==================================\n");
      return trimmed;
    }
    Serial.println("ERROR: No 'text' field in response and body empty!");
...
// Optional: upload a JSON log to Azure Blob Storage using a SAS token.
// This is intentionally simple: if AZURE_BLOB_CONTAINER_URL or AZURE_BLOB_SAS
// are empty the function returns false immediately and does not affect main flow.
// Insert a log entity into Azure Table Storage using SAS auth.
// No-op (returns false) if AZURE_TABLE_URL or AZURE_TABLE_SAS is empty.
bool uploadLogToTable(const String &transcription, const String &assistantResponse) {
  if (strlen(AZURE_TABLE_URL) == 0 || strlen(AZURE_TABLE_SAS) == 0) return false;

  // Build entity JSON. Using minimal metadata format for Table REST API.
  // PartitionKey and RowKey are required.
  unsigned long t = millis();
  int r = random(10000, 99999);
  String rowKey = String(t) + "-" + String(r);

  String payload = "{";
  payload += "\"PartitionKey\":\"" + String(DEVICE_ID) + "\",";
  payload += "\"RowKey\":\"" + rowKey + "\",";
  payload += "\"TimestampMs\":\"" + String(t) + "\",";
  payload += "\"transcription\":\"" + escapeJSON(transcription) + "\",";
  payload += "\"assistantResponse\":\"" + escapeJSON(assistantResponse) + "\"";
  payload += "}";

  // Build request URL: <tableServiceUrl>/<tableName>?<sas>
  String base = String(AZURE_TABLE_URL);
  if (base.endsWith("/")) base.remove(base.length() - 1);
  String sas = String(AZURE_TABLE_SAS);
  if (sas.startsWith("?")) sas = sas.substring(1);
  String url = base + "/" + String(AZURE_TABLE_NAME) + "?" + sas;

  Serial.println("Inserting entity to Table: " + url);
  ...
  http.addHeader("Prefer", "return-no-content");

  int code = http.POST(payload);
  String resp = http.getString();
  http.end();

  Serial.printf("Table insert HTTP %d\n", code);
  if (code == 201 || code == 204) {
    return true;
  }

  Serial.println("Table insert failed: " + resp);
  return false;
}
...
String askGPT(const String &question) {
  ...
  // Escape special characters in question for safe JSON embedding
...
void loop() {
  ...
  if (M5.BtnA.wasClicked()) {
    ...
    Serial.println(response);
    // Optionally upload the exchange to Azure Table Storage (non-blocking on failure)
    if (!uploadLogToTable(question, answer)) {
      Serial.println("Table logging disabled or failed; continuing.");
    } else {
      Serial.println("Logged conversation to Azure Table.");
    }
    
    // Prepare pages and show first page
    prepareResponsePagination(response);
    drawAnswerPage();
  ...
  // ✨ BUTTON C (power): Advance page when viewing a long answer (no-op otherwise)
  if (M5.BtnPWR.wasClicked()) {
    if (showingAnswer && totalPages > 1) {
